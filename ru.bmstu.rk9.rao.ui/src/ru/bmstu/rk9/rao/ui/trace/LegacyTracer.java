...
public class LegacyTracer implements SimulatorDependent {
	...
	public LegacyTracer(SimulatorId simulatorId) {
	...
	public final void parseAllEntries() {
		final List<Entry> entries = getSimulatorWrapper().getDatabase().getAllEntries();
	...
	protected final String parseResourceParameters(final ByteBuffer data, final int typeNum) {
		...
		final int numberOfParameters = getSimulatorWrapper().getStaticModelData().getNumberOfResourceTypeParameters(typeNum);
		final int finalOffset = getSimulatorWrapper().getStaticModelData().getResourceTypeFinalOffset(typeNum);

		for (int paramNum = 0; paramNum < numberOfParameters; paramNum++) {
			DataType dataType = getSimulatorWrapper().getStaticModelData().getResourceTypeParameterType(typeNum, paramNum);
			switch (dataType) {
			case INT:
			...
			// break;
			case OTHER:
				final int index = getSimulatorWrapper().getStaticModelData().getVariableWidthParameterIndex(typeNum,
						paramNum);
	...
	protected final String parsePatternData(final ByteBuffer data, final TraceType patternType) {
		...
		for (int num = 0; num < numberOfRelevantResources; num++) {
			final int typeNum = getSimulatorWrapper().getStaticModelData().getRelevantResourceTypeNumber(patternNumber, num);
	...
	protected TraceOutput parseSearchEntry(final Entry entry) {
		...
		case SPAWN: {
			...
			final int numberOfRelevantResources = getSimulatorWrapper().getStaticModelData()
			...
			for (int num = 0; num < numberOfRelevantResources; num++) {
				final String typeName = getSimulatorWrapper().getStaticModelData().getRelevantResourceTypeName(patternNumber,
						num);
				final int typeNum = getSimulatorWrapper().getStaticModelData().getResourceTypeNumber(typeName);
				final int resNum = data.getInt();
	...
	private final void initializeTypes() {
		final int numberOfResourceTypes = getSimulatorWrapper().getStaticModelData().getNumberOfResourceTypes();
	...
	private final void initializeActivities() {
		final int numberOfLogics = getSimulatorWrapper().getStaticModelData().getNumberOfLogics();
		...
		for (int dptNum = 0; dptNum < numberOfLogics; dptNum++) {
			...
			int numberOfActivities = getSimulatorWrapper().getStaticModelData().getNumberOfActivities(dptNum);
